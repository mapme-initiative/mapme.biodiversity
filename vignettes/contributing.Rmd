...
If you are reading this vignette you most probably to contribute to the `{mapme.biodiversity}`
...
information about how to add resources and indicators in order to make the process
...
vignette in order to get familiar with the most important concepts of this package.
...
When submitting a PR which does not consistently follow the tidyverse style guide, 
...
The general guidelines to follow also apply if you choose a different tooling 
...
add a resource is to add it into the internal resource backlog function so
...
see that each resource shares a common structure how it is specified. The name
of the list object will be the name how the package identifies a specific resource.
...
arguments are needed just enter a empty list. For the `soilgrids` resource,
the internal backlog looks like this (don't spent too much effort in understanding
...
function actually works when called by users. 
...
sure to include a detailed documentation. This documentation should explain
what this resource represents, where it comes from (including a citation), 
and the arguments users should specify to control what is actually downloaded.
...
actual downloader. The downloader is a package internal function that users do not
...
a output directory on disk where the output shall be written to when used by users.
Additionally, a logical called `verbose`, by default set to TRUE, controlling 
...
downloader requires. The package framework does not have the capability to check
...
misspecified and which values actually represent valid values. You can head
...
and the portfolio-wide parameters the files matching these arguments are downloaded.
...
If you construct a number of URLs and associated local filenames that you wish
to iterate over, the package provides a helper function to download these files
and skip already exisiting ones. The following code snippet shows how
...
to TRUE. The stubbornnes controls the number of retries of failed downloads.
If check_existence is set to TRUE, `RCurl::url.exists(url)` will be used
to check if a given URL actually exists online. This check takes some time to
run, but for some resources it can be useful if you cannot be sure if the constructed
URL actually exists on the remote location. Users are enabled to specify
...
increase the understandibility and ease maintenance of your downloader. Any helper
functions associated with your resource should be located in the same file. If 
you feel a helper function might server a purpose across different resource,
...
to include a documentation to make your code easier to understand, make sure to
add the `@keywords internal` and `@noRd` tags to your functions.

## Defining the output and handling intermediat files
...
including the local filepaths to all target files matching the spatio-temporal
extent of the portfolio `x`. For raster resources, a tileindex indicating the
...
Note, that a resources **SHALL NOT** add additional dependencies to the package. If
you add a dependencies we require you to add a supporting statement to your PR
explaining why this dependency is needed and why other approaches would fail.
Before accepting your PR, we might request you to change your code to remove 
that dependency, if it is feasible to achieve the same functionality without it.
