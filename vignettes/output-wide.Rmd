...
aoi <- get_resources(aoi, get_nelson_et_al(ranges = "20k_50k"))
...
```{r calc-prec}
aoi <- get_resources(aoi, get_worldclim_precipitation(years = 2010:2018))
...
might be very memory intensive. In these cases it might be more favorable to 
...
dsn_long <- tempfile(fileext = ".gpkg")
dsn_wide <- tempfile(fileext = ".gpkg")
write_portfolio(select(aoi, traveltime), dsn_long, format = "long", quiet = TRUE)
write_portfolio(select(aoi, traveltime), dsn_wide, format = "wide", quiet = TRUE)
```
