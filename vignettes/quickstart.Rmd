...
aoi_gridded <- calc_indicators(
  aoi_gridded,
  calc_popcount(engine = "zonal", stats = "sum")
)
```
...
```{r select_cols}
(aoi_gridded <- aoi_gridded %>% select(WDPAID, popcount))
```
...
```{r unnest}
geometries <- select(aoi_gridded, WDPAID)
aoi_gridded %>%
  st_drop_geometry() %>%
