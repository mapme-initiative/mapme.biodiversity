...
This vignette assumes that you have already followed the steps in [Installation](installation.html) and have familiarized yourself with the 
terminology used in the package. If you are unfamiliar with the terminology used here, please head over to the [Terminology](terminology.html)
...
by default. If you use another operating system you may set to a value greater 1 if you wish to activate parallel 
processing. Finally, the `verbose` logical controls the verbosity 
...
over to the [chirpsprec](../reference/chirpsprec.html) documentation.
...
to calculate the `chirpsprec` indicator which is based on the `chirps` resource.
Since the resource has been made available in the previous step, we can continue
requesting the calculation of our desired indicator. Note, the below
...
normal period (make sure to read the detailed indicator documentation via `?chirpsprec`).
...
sample_portfolio %>%
  ...
  labs(
    title = "Monthly precipitation anomaly 2010", x = "", y = "Precipitation anomaly [mm]",
    fill = "Anomaly"
  ) +
  theme_classic() +
