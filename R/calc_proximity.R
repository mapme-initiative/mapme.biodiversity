...
#' Calculate Proximity to Protected Areas from WDPA
#'
#' This function calculates the distance to the closest WDPA within an area
#' of influence around an assets location.
#'
#' The required resources for this indicator are:
#'  - [wdpa]
...
#' st_geometry(aoi) <- "buffer"
#'
#' aoi <- get_resources(aoi, get_wdpa(version = "Oct2024"))
#' aoi <- calc_indicators(aoi, calc_proximity_wdpa(asset_column))
#'
#' aoi$buffer <- NULL
#' st_geometry(aoi) <- asset_column
#'
...
register_indicator(
  name = "proximity_wdpa",
  description = "Distance (in meters) to the closest WDPA within an area of influence.",
  resources = "wdpa"
)
