...
.calc_treeloss <- function(shp,
  ...
  minCover_msg <- paste("Argument 'minCover' for indicator 'treeloss' ",
  ...
  minSize_msg <- paste("Argument 'minSize' for indicator 'treeloss' ",
  ...
  yearly_loss_values <- lapply(years, function(y) {
    y <- y - 2000
    current_treecover <- ifel(
      lossyear <= y, NA, binary_treecover,
      filename = ifelse(todisk, file.path(rundir, "current_treecover.tif"), ""),
      datatype = "INT1U",
      overwrite = TRUE
    )
    current_arearaster <- mask(
      arearaster, current_treecover,
      maskvalues = c(NA, 0),
      filename = ifelse(todisk, file.path(rundir, "current_area.tif"), ""),
      datatype = "FLT4S",
      overwrite = TRUE
    )
    ha_sum_treecover <- zonal(
      current_arearaster,
      polyraster, sum,
      na.rm = TRUE
    )[2]
    ha_sum_treecover <- as.numeric(ha_sum_treecover)
    ...
    tibble(treecover = ha_sum_treecover, emissions = emissions_sum)
  })
