...
.calc_treeloss <- function(shp,
  ...
  if(any(missing(treecover), missing(lossyear), missing(greenhouse))){
    return(tibble(years = years, treecover = rep(NA, length(years)), emissions =  rep(NA, length(years))))
  }
  ...
  if(all(length(unique(unlist(minmax(treecover)))) == 1, unique(unlist(minmax(treecover))) == 0)){
    return(tibble(years = years, treecover = rep(0, length(years)), emissions =  rep(0, length(years))))
  }
